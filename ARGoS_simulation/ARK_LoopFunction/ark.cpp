 ...
 * @file <ark.h>
 *
 * @brief This is the source file of ALF, the ARK (Augmented Reality for Kilobots) loop function. Here, we reproduce
...
/****************************************/
CArk::CArk(): CLoopFunctions(),m_unMessagingCounter(0), m_unDataSavingCounter(1),m_bDynamicVirtualEnvironments(false),
    m_unEnvironmentPlotUpdateFrequency(10),m_unDataFrequency(100),m_unOhcFrequency(10),
    m_fQuorum(1.0),m_unTau0(0),m_bQuorumReached(false),m_bOptionsLookUpTableSent(false),m_unOptionsSent(0){}
CArk::~CArk(){
}
...
void CArk::SetupVirtualEnvironments(TConfigurationNode& t_tree){
    ...
    if(type=="automatic"){
        ...
        for(unsigned int i=1; i<=numberofoptions;i++){
            ...
            sOption.position=CVector2(polygonradius*Cos(angleoffset+(numberofoptions-i)*CRadians::TWO_PI/numberofoptions)+1,polygonradius*Sin(angleoffset+(numberofoptions-i)*CRadians::TWO_PI/numberofoptions)+1);
...
void CArk::GetExperimentVariables(TConfigurationNode& t_tree){
    ...
    LOG << "Noise variance= " << m_fvariance << std::endl;

    /* Get tau0  value */
    GetNodeAttributeOrDefault(tExperimentVariablesNode, "tau0", m_unTau0, m_unTau0);
    LOG << "tau0= " << m_unTau0 << std::endl;
...
void CArk::UpdateVirtualSensors(){
    ...
    /* Update flags */
    if(m_unOptionsSent<m_tOptions.size()){
        m_unOptionsSent++;
    }
...
void CArk::UpdateVirtualSensor(CKilobotEntity &c_kilobot_entity){
    ...
    if(!m_bOptionsLookUpTableSent){
        /* Prepare the message */
        m_tMessages[GetKilobotId(c_kilobot_entity)].type=1;
        m_tMessages[GetKilobotId(c_kilobot_entity)].data[0]=m_tOptions[m_unOptionsSent].id;
        ...
        m_tMessages[GetKilobotId(c_kilobot_entity)].data[3]=(UInt8) (m_fvariance*10);
        m_tMessages[GetKilobotId(c_kilobot_entity)].data[4]= m_unTau0;
        ...
        */
        ...
        if( ( tEncounteredOption[GetKilobotId(c_kilobot_entity)] != -1) && (m_fTimeInSeconds - tLastTimeMessagedDiscovery[GetKilobotId(c_kilobot_entity)]> MinTimeBetweenTwoMsg ))
        {
            ...
            UInt8 un_QualityToSend=(UInt8) m_tOptions[ tEncounteredOption[GetKilobotId(c_kilobot_entity)] ].quality;;

            if( m_tOptions[ tEncounteredOption[GetKilobotId(c_kilobot_entity)] ].quality>9.0 && m_tOptions[ tEncounteredOption[GetKilobotId(c_kilobot_entity)] ].quality<10.0 )
            {
                un_QualityToSend += 1;
                un_QualityToSend += (10*m_tOptions[ tEncounteredOption[GetKilobotId(c_kilobot_entity)] ].quality - 10* (UInt8) m_tOptions[ tEncounteredOption[GetKilobotId(c_kilobot_entity)] ].quality );

            }

            /* Fill the message */
            tKilobotMessage.m_sID = GetKilobotId(c_kilobot_entity) << 2 | b_MessageType << 1 |  ( (UInt8) m_tOptions[ tEncounteredOption[GetKilobotId(c_kilobot_entity)] ].GPS_position.GetX() ) >> 4;
            tKilobotMessage.m_sType = ( (UInt8) m_tOptions[ tEncounteredOption[GetKilobotId(c_kilobot_entity)] ].GPS_position.GetX() ) & 0x0F;
            tKilobotMessage.m_sData = ( (UInt8) m_tOptions[ tEncounteredOption[GetKilobotId(c_kilobot_entity)] ].GPS_position.GetY() ) << 5 | un_QualityToSend  ;
        ...
        if((!bMessageToSend)&&tRequiresGPS[GetKilobotId(c_kilobot_entity)]&& (m_fTimeInSeconds - tLastTimeMessagedGPS[GetKilobotId(c_kilobot_entity)]> MinTimeBetweenTwoMsg ) )
        {
            ...
            /* Fill the message */
            tKilobotMessage.m_sID = GetKilobotId(c_kilobot_entity) << 2 | b_MessageType << 1 |  ( (UInt8) PositionToGPS(GetKilobotPosition(c_kilobot_entity)).GetX()) >> 4;
            ...
            tKilobotMessage.m_sData = ( (UInt8) PositionToGPS(GetKilobotPosition(c_kilobot_entity)).GetY() ) << 5 | ( (UInt8) (KB_Orientation.GetValue()/12.0) ) ;
