...
void update_social_parameters(){
    double tau_max=tau_in_minutes*60.0*30.0*(100.0/my_option_quality);
    double H=100.0;

    if(kilo_ticks<=tau_max) h=H*kilo_ticks/tau_max;
    else h=H;
}
...
void loop() {
    ...
    // Send ARK LoopFunction information about commitement (for logging purpose)
